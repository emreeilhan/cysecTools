            ...
            <div className="px-5 pb-5 pt-1 border-t border-neutral-200 dark:border-neutral-800">
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  <SectionTitle>{t("common_commands")}</SectionTitle>
                  <CopyButton
                    label={t("copy_all")}
                  ...
                  {tool.commands.map((sec, idx) => (
                    <div key={idx} className="rounded-lg border border-neutral-200 dark:border-neutral-800 overflow-hidden">
                      ...
                      <div className="p-3 space-y-2 bg-white/60 dark:bg-neutral-950/40">
                        {sec.entries.map((e, i) => (
                          <CodeRow key={i} label={e.label} cmd={e.cmd} onCopy={() => copyText(e.cmd, `${idx}-${i}`)} copied={copiedIdx === `${idx}-${i}`} />
                        ))}
                      </div>
                    </div>
                  ))}
              ...
              {/* Guided Recipe */}
              {Array.isArray(tool.recipe) && tool.recipe.length > 0 ? (
                <div className="mt-6">
                  <SectionTitle>{t("guided_recipe")}</SectionTitle>
                  <ol className="relative mt-2 ml-3" role="list" aria-label={`${tool.name[lang]} recipe`}>
                    {/* Purpose: vertical connection line between steps */}
...
// Small helpers to generate sample file contents
const genYara = (lang) => `// ${lang === "tr" ? "Basit YARA kuralı" : "Simple YARA rule"}
...
const genSigmaSysmon = (lang) => `title: Possible PowerShell Download
...
const genSysmonConfig = (lang) => `<!-- ${lang === "tr" ? "Minimum Sysmon konfig (örnek)" : "Minimal Sysmon config (example)"} -->
...
const genSuricataRule = (lang) => `alert http any any -> any any (msg:"${lang === "tr" ? "Şüpheli User-Agent" : "Suspicious User-Agent"}"; content:"curl/"; http_header; classtype:policy-violation; sid:1000001; rev:1;)`;

const genINetSimConf = (lang) => `# ${lang === "tr" ? "INetSim örnek konfig" : "INetSim sample config"}
...
const genIOCList = (lang) => `# ${lang === "tr" ? "Gözlenecek IOC'ler" : "IOCs to watch"}
8.8.8.8
1.2.3.4
